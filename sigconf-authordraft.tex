...
Drawing on semantic content-unit and relation-based approaches to summary evaluation~\cite{nenkova-passonneau-2004-evaluating,hovy-etal-2006-automated}, we developed a study-specific, structure-aware analysis. To construct reference targets for each paper, two authors independently extracted core claims from the source introductions (29 cores for \textit{Synthetic Lies}~\cite{synthetic_lies}; 22 for \textit{Why Johnny Can't Prompt}~\cite{Zamfirescu-Pereira}). We retained claims independently identified by both authors as the reference set.

We then segmented each participant's written summary into semantic units (typically sentence- or claim-level statements) and coded every unit against the corresponding paper's core set and introduction. Coding was performed by the same two authors---not by external domain experts. Units from both system conditions were interleaved and scored without condition labels visible during judgment (blind unit review). Each unit received one of three verdicts: \textit{accurate} (supported by and mapped to one or more cores), \textit{inaccurate}/\textit{hallucinated} (unsupported by or inconsistent with the source)~\cite{maynez-etal-2020-faithfulness}, or \textit{too vague} to credit (excluded from precision and hallucination denominators). Based on these annotations, we computed three descriptive measures per summary:
\begin{itemize}[leftmargin=*, noitemsep]
    \item \textbf{Recall}: distinct core claims covered / number of cores for that paper;
    \item \textbf{Precision}: accurate participant units / non-vague participant units;
    \item \textbf{Hallucination}: inaccurate participant units / non-vague participant units.
\end{itemize}

We analyze these scores at two complementary grains. At the \emph{participant} level, we compare paired condition means with Wilcoxon signed-rank tests ($n=12$), treating each reader as the independent unit. At the \emph{pooled-unit} level, we additionally report $\chi^2$ tests on contingency tables that pool accurate vs.\ inaccurate units (and covered vs.\ uncovered cores) across participants; these tests are descriptive of unit counts and do not treat nested units as independent observations. Because constructing cores, segmenting summaries, and judging correspondence involve interpretive decisions, we use the measures primarily as a quality guardrail on whether the observed behavioral shift came with an evident factuality trade-off, rather than as a standalone reading-performance score.

\subsubsection{Annotation Reliability}

Agreement was higher for individual semantic units than for whole-summary judgments. For hallucination judgments at the unit level, the two author-annotators achieved 92.6\% raw agreement with Cohen's chance-corrected agreement coefficient~\cite{cohen1960coefficient} ($\kappa=.535$). Agreement on whether participant units corresponded to core reference points was likewise $\kappa\approx.60$, depending on the annotation criterion. We also observed a systematic difference in annotation tendencies, with one author making significantly more hallucination judgments than the other (McNemar $\chi^2=6.67$, $p=.010$). At the summary level, agreement was substantially lower: when annotators judged whether an entire participant summary should be considered hallucinatory, raw agreement decreased to 54.2\% ($\kappa=.083$). We therefore report quantitative summary-quality measures from unit-level annotations (Table~\ref{tab:summary_quality} reflects the coding used for primary reporting), treat whole-summary labels cautiously, and interpret pooled-unit $\chi^2$ in light of annotator-strictness differences.
...
\caption{Coded behaviors along the session timeline (real time, 0--100\%; $n=12$). The Baseline condition is shown on the left and the StructWeaver condition on the right. Each row is one participant; colored marks show the approximate session progress at which each behavior occurred. The bottom Overall row semi-transparently overlays all events.}
...
\caption{Behavior counts in 10\% session bins ($n=12$). Solid lines show per-category means across participants; shaded bands indicate $\pm 1$ SD. Under the StructWeaver condition, source grounding concentrates in the mid-to-late session; under the Baseline condition, summarization/compression more often appears in the first half.}
...
\subsection{Summary Quality: Participant-Level Tests Were Conservative; Unit Pooling Was Coder-Sensitive}
\label{sec:summary-quality-results}

As a descriptive quality guardrail, we computed Recall, Precision, and Hallucination for each participant's written summary under the StructWeaver and Baseline conditions (definitions in \S\ref{sec:summary-eval}; per-participant scores in Table~\ref{tab:summary_quality}). Two author-annotators coded summary units blind to system condition; Table~\ref{tab:summary_quality} reports the coding used for primary participant-level analysis.
...
\paragraph{Pooled unit level.}
Pooling the same unit annotations across participants, contingency tables associating condition with accurate vs.\ inaccurate units yielded $\chi^2=10.77$, $p=.001$ (Baseline: 69 accurate / 14 inaccurate; StructWeaver: 98 / 2), and associating condition with covered vs.\ uncovered cores yielded $\chi^2=6.46$, $p=.011$ (Baseline: 69 covered / 237 uncovered; StructWeaver: 98 / 208). Hallucination at this grain is the complement of the precision table and therefore shares the same $\chi^2$. An independent second-author coding of the same interleaved units did \emph{not} show significant condition associations (precision/hallucination $\chi^2=0.03$, $p=.85$; recall $\chi^2=3.20$, $p=.074$), consistent with the systematic annotator-strictness difference in \S\ref{sec:summary-eval}. Because units are nested within participants, we treat pooled $\chi^2$ as descriptive of unit counts rather than as a substitute for the paired reader-level tests. Taken together, the conservative participant-level Wilcoxon results and the coder-sensitive unit pooling suggest that the shift from summary consumption toward source grounding did not come with a clear, coder-robust cost---or advantage---on headline coverage and factuality metrics.

\begin{table*}[t]
\centering
\caption{Per-participant summary quality under the StructWeaver and Baseline conditions ($n=12$). Cells are counts (hits/denominator). Recall denominators are core claims (22 or 29 depending on the assigned paper); Precision and Hallucination denominators are non-vague participant units in that summary (two authors; blind unit coding).}
...
\subsection{Summary Evaluation Was More Reliable at the Unit Level}

Our summary evaluation also revealed substantial differences in annotation reliability depending on the granularity of the judgment. At the level of individual summary units, the two author-annotators were generally able to determine whether participant-generated content was supported by the paper. Hallucination judgments achieved 92.6\% raw agreement with Cohen's $\kappa=.535$, while agreement on correspondence to core reference points was $\kappa\approx.60$, depending on the annotation criterion. We nevertheless observed systematic differences between the two authors. One made significantly more hallucination judgments than the other (McNemar $\chi^2=6.67$, $p=.010$), indicating that even local factuality judgments depended partly on how strictly unsupported interpretations were defined---and helping explain why pooled-unit $\chi^2$ results differed across the two independent codings. Agreement decreased substantially when judgments were made at the level of an entire participant summary. For whether a complete summary should be considered hallucinatory, raw agreement was 54.2\%, with Cohen's $\kappa=.083$. Thus, while the authors could more consistently assess whether individual units were supported by the source, they frequently disagreed when characterizing the participant's overall interpretation. Given this difference, we rely primarily on unit-level annotations when using written summaries as a descriptive quality guardrail, keep participant-level Wilcoxon tests as the conservative comparison, and treat aggregate judgments about overall understanding more cautiously (Section~\ref{sec:discussion}).
...
\begin{itemize}[leftmargin=*, noitemsep]
  \item Towards Lifelong Dialogue Agents via Timeline-based Memory Management
  \item Selenite: Scaffolding Online Sensemaking with Comprehensive Overviews Elicited from Large Language Models
  \item Structuring Knowledge with Cognitive Maps and Cognitive Graphs
  \item Spellburst: A Node-based Interface for Exploratory Creative Coding with Natural Language Prompts
  \item Synergi: A Mixed-Initiative System for Scholarly Synthesis and Sensemaking~\cite{Synergi}
  \item The Story in the Notebook: Exploratory Data Science using a Literate Programming Tool
  ...
  \item Towards an instrument for measuring sensemaking and an assessment of its theoretical features
  \item mage: Fluid Moves Between Code and Graphical Work in Computational Notebooks
  \item PaperWeaver: Enriching Topical Paper Alerts by Contextualizing Recommended Papers with User-collected Papers
  \item Patterns of Hypertext-Augmented Sensemaking
  \item Positional Control in Node-Based Programming
  \item Opening up the black box of scholarly synthesis: Intermediate products, processes, and tools
  \item Relatedly: Scaffolding Literature Reviews with Existing Related Work Sections
  \item ScholarMate: A Mixed-Initiative Tool for Qualitative Knowledge Work and Information Sensemaking
  \item Leveraging Large Language Models for Realizing Truly Intelligent User Interfaces
  \item A Similarity Measure for Comparing Conversational Dynamics
  \item Closing the Loop: Learning to Generate Writing Feedback via Language Model Simulated Student Revisions
  \item Developing a Model of Distributed Sensemaking: A Case Study of Military Analysis
  \item Rapsai: Accelerating Machine Learning Prototyping of Multimedia Applications through Visual Programming
  \item Effects of Implicit Sharing in Collaborative Analysis
  \item eRevise+RF: A Writing Evaluation System for Assessing Student Essay Revisions and Providing Formative Feedback
  \item Graphologue: Exploring Large Language Model Responses with Interactive Diagrams~\cite{graphologue}
  \item Interactive Visual Analytics for Sensemaking with Big Text
\end{itemize}
...
Two authors independently extracted core claims from each source introduction (29 for \textit{Synthetic Lies}; 22 for \textit{Why Johnny Can't Prompt}) and retained their intersection as the reference set. Participant summaries were segmented into semantic units and scored blind to system condition for coverage and factual support (accurate / inaccurate / too vague).
