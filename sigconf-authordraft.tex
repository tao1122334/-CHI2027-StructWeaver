...
\caption{Study interfaces shown to participants. \textbf{Left (Baseline):} a participant's habitual chat-based LLM alongside a PDF viewer (representative screenshot); AI responses and source text remain in separate panes. \textbf{Right (StructWeaver):} hierarchical node workspace with gist--verbatim alignment, verbatim panel, and AI response panel.}
...
\paragraph{Workflow.}
P08 opened the assigned PDF and provided it to a familiar commercial chat LLM, then requested a plain-language overview organized around background, problem, methods, findings, and contributions. The model returned a multi-section summary that made the paper's scope legible at a high level.
