...
Existing AI-assisted reading systems have substantially improved how readers access and inspect long documents~\cite{semantic_reader,scholarphi,paper_plain}. Overview systems provide adaptive summaries and explanations~\cite{qlarify,concepteva,abstractexplorer}, provenance-oriented systems expose supporting passages or citations~\cite{papertrail,tabletale,kambhamettu2024traceabletextdeepeningreading}, and sensemaking tools help readers externalize and organize information~\cite{liquidtext,sensecape,pirolli2005sensemaking}. Nevertheless, AI-generated abstractions and verbatim source evidence typically remain separate representations. Even when supporting evidence is available, readers often need to interrupt their current reading process, locate the relevant source passage, and reconstruct how it corresponds to an earlier AI interpretation~\cite{papertrail,vered2023}. This separation becomes particularly consequential when the AI's organization of a document differs from the author's argument or the reader's own emerging understanding. We therefore focus on a structural gap in current AI-assisted reading workflows: readers can obtain AI-generated gist and access source evidence, but lack a persistent and recoverable path for moving between them during reading.
...
Based on these findings, we present \textit{StructWeaver} (Figure~\ref{fig:teaser}), an interactive AI-assisted reading system designed to make AI-generated abstractions traversable. StructWeaver introduces \textit{Gist--Verbatim alignment}, which decomposes generated gist into smaller, revisitable units and associates each unit with order-preserving source evidence~\cite{kambhamettu2024traceabletextdeepeningreading}. The system provides three forms of support corresponding to the breakdowns identified in our formative study. First, a hierarchical node workspace externalizes AI-generated gist as decomposable interpretive units rather than monolithic responses~\cite{davies2011mind,sensecape}. Second, explicit Gist--Verbatim correspondence allows readers to move directly between an abstraction and its associated source passages. Third, follow-up questions are attached to persistent interpretation branches rather than appended to a global chat transcript, allowing readers to recover and extend previous lines of inquiry~\cite{ai_chains,threddy}. StructWeaver preserves familiar chat-and-PDF interactions while reorganizing them around the relationships among AI interpretations, author arguments, and readers' evolving understanding.

We further conducted a paired within-subjects study with 12 participants to compare StructWeaver with a conventional linear chat-and-PDF workflow. Behavioral logs showed that StructWeaver shifted participants' interactions away from repeated summarization and compression and toward source grounding: participants more frequently traversed from generated gist to source passages during ordinary reading. They used these transitions to inspect how AI compressed the source, notice potentially incomplete support, and repair mismatches between AI-generated interpretations and author arguments. Questionnaire results further showed lower intrinsic and extraneous load and higher germane load with StructWeaver~\cite{sweller2011cognitive}, suggesting that reducing the effort required to coordinate disconnected representations allowed participants to devote more effort to organizing and integrating the material. Our descriptive summary-quality evaluation showed no stable significant system advantage, suggesting that these behavioral changes did not introduce an evident trade-off in headline coverage or factuality measures.
...
Three lines of work have substantially improved how readers enter, inspect, and organize long documents with AI assistance~\cite{semantic_reader,scholarphi,paper_plain,papertrail,liquidtext,sensecape}. Yet across all three, AI-generated abstractions and verbatim source evidence remain structurally separate: readers may access evidence through additional effort, but the path between abstraction and source is never built into the reading act itself. We review each line in turn to show where this structural gap originates and why filling it requires a different kind of representation.
...
HCI research has long explored ways to externalize understanding through annotations, spatial organization, and persistent structures. Systems such as \textit{LiquidText}, \textit{Garden of Papers}, and \textit{CrossLit} support excerpt collection, linking, and organization across documents~\cite{liquidtext,garden_of_papers,crosslit}. More recent AI-assisted systems, including \textit{Sensecape}, \textit{Graphologue}, \textit{DiscipLink}, \textit{AI Chains}, and \textit{Tree of Analysis}, extend this direction by externalizing model interactions through diagrams, multilevel exploration, and structured workflows~\cite{sensecape,graphologue,disciplink,ai_chains,tree_of_analysis}.

These systems demonstrate that making intermediate artifacts persistent and manipulable supports complex information tasks~\cite{pirolli2005sensemaking}. However, the artifacts they externalize are user-authored notes, AI workflows, or inter-document relationships, not the structural relationship between an AI-generated interpretation and the verbatim source it abstracts over. This leaves the gist--verbatim binding unrepresented in the reading environment: readers can organize what they understand, but they cannot recover how that understanding relates to what the author actually wrote.

\subsection{Toward Gist--Verbatim Alignment}
\label{sec:gist-verbatim}

Taken together, the three lines of work above reveal a structural gap. Systems that generate AI overviews lower the entry barrier to dense documents, but present abstraction as a finished output, leaving its connection to the underlying source implicit. Provenance-based approaches surface evidence and can calibrate readers' trust, yet as the trust--behavior gap suggests, \textit{available} evidence and \textit{consulted} evidence are not the same thing: when evidence links require extra effort to reach, they remain optional audits rather than natural moves within reading. Externalization systems make intermediate artifacts persistent and manipulable, but the artifacts they structure are user-authored notes and AI workflows, not the binding between an AI-generated interpretation and the verbatim text it rests on.

What is missing across all three lines is not another reading component, but a recoverable path: a structural binding that lets readers move from AI-generated gist down into the verbatim evidence it rests on, and back up into their own interpretive path, as part of the same reading act. Without this path, verification remains a separate activity, triggered only by explicit doubt, rather than a continuous movement within sensemaking~\cite{pirolli2005sensemaking,vered2023}.

\textit{StructWeaver} is built around this missing path. It builds on all three lines by generating AI gist as an entry point, exposing linked source evidence, and externalizing gist as a manipulable structure, while adding the one capability none of the three lines provides: a recoverable path from AI-generated gist to verbatim source evidence. Rather than introducing new reading primitives, it reconfigures familiar chat-and-PDF workflows by decomposing AI-generated interpretations into manipulable gist units and structurally aligning each unit with order-preserving source evidence. The result is a verbatim entry point that is always one step away from the reader's current gist focus, making it possible to consult the source during understanding and return to abstraction without losing one's place. Table~\ref{tab:rw_comparison} summarizes how each line of work supports these four capabilities.
...
\begin{table*}[t]
\centering
\caption{Systematic comparison of AI-assisted reading baselines. While most current tools successfully provide an ``AI Gist'' (high-level, AI-generated summaries or synthesized interpretations of the source text), our capability mapping highlights a critical structural gap in continuous verification. \textit{StructWeaver} emerges directly from this analysis to provide the missing dimensions: a recoverable gist--verbatim path and persistent context.}
\label{tab:rw_comparison}
\small
...
\textit{ScholarPhi}~\cite{scholarphi}, \textit{Paper Plain}~\cite{paper_plain} & \checkmark & -- & -- & -- & -- \\
\midrule
\textbf{Provenance Systems} \\
\textit{Qlarify}~\cite{qlarify}, \textit{PaperTrail}~\cite{papertrail} & \checkmark & \checkmark & -- & -- & -- \\
\midrule
\textbf{Sensemaking Systems} \\
\textit{LiquidText}~\cite{liquidtext}, \textit{Sensecape}~\cite{sensecape} & -- & -- & \checkmark & -- & \checkmark \\
\midrule
\rowcolor[HTML]{F2F7FC} 
\textbf{\textit{StructWeaver}} & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark \\
\bottomrule
\end{tabularx}
\end{table*}
...
\midrule
\textbf{Capacity Limit} & ``Once the context window is exceeded, AI compresses previous history and sometimes loses context.'' (T8) \\
\textbf{Coarse Gist} & ``AI dumps everything at once. Sometimes I only want supplementary ideas, and sometimes I need deeper verification.'' (T1) \\
\textbf{Disconnected Interfaces} & ``Copying text into ChatGPT every time is troublesome, and important context often gets lost.'' (T1) \\
\textbf{Ephemeral Paths} & ``In many cases, each round of questioning gives me only one useful piece, but only one piece.'' (T8) \\
\textbf{Verification Collapse} & ``If ChatGPT can help me truly understand the terminology, things become easier. But if I still don't understand, I just postpone the problem.'' (T1) \\
\bottomrule
\end{tabularx}
\end{table*}
...
\begin{figure*}[t]
\centering
\includegraphics[trim=0 98 0 58, clip, width=\textwidth]{fig/gist_verbatim_foothold.pdf}
\caption{\textbf{StructWeaver overview.} Readers inspect the original source (A), navigate AI-generated gist units (B), and traverse their aligned verbatim evidence (C), with the corresponding source passage highlighted on selection. Follow-up questions produce detailed AI responses (D) grounded in the current interpretation path. StructWeaver supports this interaction by decomposing and structuring the source in advance, then dynamically composing path-specific context during reading.}
...
\caption{Gist--Verbatim--Source alignment and recoverable interaction paths in StructWeaver. The left side shows explicit links among AI gist, verbatim evidence, and highlighted source passages. The right side illustrates how readers continue questioning along a branch, including linear follow-ups, splitting to explore different angles, merging related threads, and backtracking to earlier nodes.}
...
We conducted a within-subjects study with 12 participants (P01--P12), distinct from the formative study participants (T01--T12), to investigate how StructWeaver changes AI-assisted reading compared with a conventional chat-and-PDF workflow. Because StructWeaver is designed primarily to change how readers coordinate AI-generated abstractions with source evidence, we focused on observable reading processes and used subjective experience and summary quality to examine the consequences of these behavioral changes. Specifically, we asked: \textbf{RQ1:} How does StructWeaver affect how readers coordinate AI-generated gist and source evidence during reading? \textbf{RQ2:} How do these changes affect readers' reading outcomes and subjective experiences?

\subsection{Study Design}

We compared StructWeaver with a Baseline condition consisting of a conventional linear chat interface alongside a PDF viewer. Participants completed AI-assisted reading tasks using unfamiliar research papers in both conditions and subsequently wrote a summary describing their understanding of each paper. We selected unfamiliar topics to reduce the influence of prior domain knowledge and encourage participants to rely on both AI-generated interpretations and the original source during reading.

Each participant completed one reading task with StructWeaver and one with Baseline, with condition order counterbalanced across participants. Before using each interface, participants received a tutorial introducing its primary interactions. During each task, participants read the assigned paper with AI assistance and determined how to allocate their attention between AI-generated content and the source document. They then wrote a summary based on their understanding of the paper, completed a post-task questionnaire, and participated in a semi-structured interview reflecting on their reading process and experience. We collected screen recordings, written summaries, questionnaire responses, and post-task interview data for subsequent analysis.
...
To examine how participants coordinated AI-generated interpretations and source evidence (RQ1), two authors independently examined the screen recordings and coded participants' observable reading actions. The coding scheme captured six categories of activity: \textit{summarization/compression}, \textit{understanding/explanation}, \textit{source reading}, \textit{source grounding}, \textit{example/association}, and \textit{conversation management}. We define \textit{source grounding} as an action that explicitly reconnects an AI-generated interpretation with source evidence, such as navigating from a gist node to its original passage, comparing an AI-generated node with linked source sentences, or following source links provided in an AI response. Coding disagreements were discussed until consensus was reached. We compared within-participant action frequencies across StructWeaver and Baseline using Wilcoxon signed-rank tests, with Holm correction for comparisons across behavioral categories. We further examined screen recordings together with post-task interviews to characterize how participants used Gist--Verbatim traversal during interpretation.

\paragraph{Subjective Experience.}
To examine participants' perceived experience under the two workflows (RQ2), we administered an 11-item post-task questionnaire using 7-point Likert scales. The questionnaire examined intrinsic, extraneous, and germane aspects of cognitive load associated with understanding document logic, maintaining and integrating information, controlling information granularity, organizing document structure, reading with explicit concepts or questions, and overall task effort. The 11 items showed acceptable internal consistency across the study (Cronbach's $\alpha=.82$). Because responses were paired and ordinal, we compared StructWeaver and Baseline using Wilcoxon signed-rank tests. Post-task interviews were used to contextualize questionnaire responses and identify why particular interactions increased or reduced perceived effort.
...
To construct reference targets for each paper, two authors independently extracted relational triples representing core claims in the source document. Each triple captured a relation central to the paper, such as an argument, finding, causal relationship, or contribution. We retained relations independently identified by both authors as the set of core reference triples.

We then segmented participants' written summaries into semantic units and represented each unit as one or more relational triples. Two authors evaluated the resulting participant triples along two dimensions: 1) whether a triple corresponded to a core reference relation, and 2) whether it contained information that was unsupported by or inconsistent with the source paper. Based on these annotations, we computed three descriptive measures:
\begin{itemize}[leftmargin=*, noitemsep]
    \item \textbf{Recall}: the proportion of core reference triples covered by the participant summary;
    \item \textbf{Precision}: the proportion of participant-generated triples that accurately reflected the source paper;
    \item \textbf{Hallucination}: the proportion of participant-generated triples containing unsupported or inaccurate information.
\end{itemize}

Because constructing reference triples, segmenting summaries, and judging semantic correspondence involve interpretive decisions, we use these measures primarily to examine whether the behavioral changes observed under StructWeaver were accompanied by evident trade-offs in coverage or factual accuracy, rather than as a standalone measure of reading performance.

\subsubsection{Annotation Reliability}

We further examined inter-rater agreement for the summary annotations. Agreement was higher when judgments were made at the level of individual semantic relations than at the level of an entire summary. For hallucination judgments at the triple level, the two annotators achieved 92.6\% raw agreement with Cohen's $\kappa=.535$. Agreement on whether participant triples corresponded to core reference points was similarly moderate to substantial ($\kappa\approx.60$, depending on the annotation criterion). We also observed a systematic difference in annotation tendencies, with one annotator making significantly more hallucination judgments than the other (McNemar $\chi^2=6.67$, $p=.010$). At the summary level, agreement was substantially lower. When annotators judged whether an entire participant summary should be considered hallucinatory, raw agreement decreased to 54.2\%, with Cohen's $\kappa=.083$. We therefore base the quantitative summary-quality measures primarily on unit-level annotations, where judgments were more consistently defined, and treat aggregate summary-level interpretations cautiously.
...
We present findings from behavioral logs, screen recordings, questionnaires, written summaries, and post-task interviews corresponding to our two research questions. Overall, StructWeaver changed both where participants directed their attention during AI-assisted reading and how they coordinated AI-generated interpretations with source evidence. Compared with Baseline, participants engaged less in repeated summary compression and more frequently returned to the source. They used Gist--Verbatim traversal not only to verify individual claims, but also to inspect how AI compressed the paper, identify potentially incomplete interpretations, and resolve mismatches between AI-generated structure and the author's argument. Questionnaire results further suggest that these behavioral changes reduced the effort required to coordinate disconnected representations while increasing participants' engagement in organizing and integrating the material. Summary quality (Recall, Precision, and Hallucination) showed no stable significant system advantage, indicating that these process shifts did not introduce an evident trade-off on coverage or factuality metrics.

\subsection{From Summary Consumption to Source Grounding}
\label{sec:behavior-logs}

Behavioral logs showed a clear difference in how participants interacted with AI across the two conditions (Figures~\ref{fig:behavior-timeline} and~\ref{fig:behavior-binned}). In Baseline, participants' interactions were largely organized around summarization and compression: they repeatedly asked AI to condense the paper, reformulate sections, or provide compact overviews. In StructWeaver, summarization/compression actions decreased while source-grounding actions increased. Rather than treating AI-generated responses as terminal outputs, participants more frequently traversed between generated gist and corresponding passages in the source document.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig/compact_behavior_timeline.png}
\caption{Coded behaviors along the session timeline (real time, 0--100\%; $n=12$). Left: Baseline; right: StructWeaver. Each row is one participant; colored marks show the approximate session progress at which each behavior occurred. The bottom Overall row semi-transparently overlays all events.}
...
\caption{Behavior counts in 10\% session bins ($n=12$). Solid lines show per-category means across participants; shaded bands indicate $\pm 1$ SD. Under StructWeaver, source grounding concentrates in the mid-to-late session; under Baseline, summarization/compression more often appears in the first half.}
\label{fig:behavior-binned}
\end{figure*}


We found source inspection became part of participants' ordinary reading process rather than a separate verification step. In Baseline, participants often obtained an AI response first and then separately decided whether returning to the PDF was necessary. StructWeaver instead allowed participants to move directly from an AI-generated gist node to its corresponding source passage. We observed participants repeatedly using these links while progressing through the paper, often without first expressing explicit doubt about the generated content. 
...
Participants further used Gist--Verbatim traversal when the AI's organization of a paper did not match their interpretation of the author's argument. P11 contrasted Baseline AI, where the model ``comes with its own logic'' that readers could only passively follow, with StructWeaver, where the generated structure could be compared directly with the original text. P03 similarly described treating the generated structure as a reference before returning to the paper to ``judge for myself.'' These interactions allowed participants to coordinate three perspectives during reading: the organization proposed by the AI, the argument presented by the author, and their own developing interpretation. When these perspectives diverged, participants could return to the source to identify the mismatch rather than continuing along the AI-generated interpretation by default.
...
Questionnaire responses showed that the behavioral differences observed with StructWeaver were accompanied by changes in participants' perceived cognitive experience. StructWeaver reduced intrinsic and extraneous load while increasing germane load, suggesting that participants experienced less difficulty coordinating the reading workflow while devoting more effort to organizing and integrating the material itself.

\subsubsection{Lower Intrinsic and Extraneous Load}

Participants reported lower intrinsic load with StructWeaver ($M=2.50$) than with Baseline ($M=3.69$; $\Delta=-1.19$, $p=.026$, $d=-0.74$). Extraneous load similarly decreased from $M=3.44$ in Baseline to $M=2.50$ with StructWeaver ($\Delta=-0.94$, $p=.011$, $d=-0.87$). Item-level results provide additional context for these differences (Table~\ref{tab:survey_stats}). Participants reported less difficulty understanding document logic and causal relationships (Q1, $p=.016$), less need to repeatedly adjust the granularity of presented information (Q6, $p=.027$), and lower overall task effort (Q11, $p=.008$). Difficulty maintaining and processing information showed a marginal difference favoring StructWeaver (Q3, $p=.059$), while Q2, Q4, Q5, and Q7 did not reach statistical significance.
...
\subsection{Summary Evaluation Was More Reliable at the Relation Level}

Our summary evaluation also revealed substantial differences in annotation reliability depending on the granularity of the judgment. At the level of individual relational triples, annotators were generally able to determine whether participant-generated content was supported by the paper. Hallucination judgments achieved 92.6\% raw agreement with Cohen's $\kappa=.535$, while judgments of correspondence to core reference points showed moderate agreement ($\kappa\approx.60$, depending on the annotation criterion). We nevertheless observed systematic differences between annotators. One annotator made significantly more hallucination judgments than the other (McNemar $\chi^2=6.67$, $p=.010$), indicating that even local factuality judgments depended partly on how strictly unsupported interpretations were defined. Agreement decreased substantially when judgments were made at the level of an entire participant summary. For whether a complete summary should be considered hallucinatory, raw agreement was 54.2\%, with Cohen's $\kappa=.083$. Thus, while annotators could more consistently assess whether individual semantic relations were supported by the source, they frequently disagreed when characterizing the participant's overall interpretation. Given this difference, we rely primarily on relation-level annotations when using written summaries as a descriptive quality guardrail. We treat aggregate judgments about participants' overall understanding more cautiously and return to the implications of this measurement difficulty in Section~\ref{sec:discussion}.
...
\subsection{From Trustworthy Outputs to Reversible Abstractions}
...
Externalizing AI-assisted reading requires balancing the expressiveness of a representation with the effort required to understand and manipulate it. Mind maps provide a relatively lightweight visual grammar of nodes and branches~\cite{davies2011mind,hollan2000distributed}, whereas argument maps and semantic graphs can encode more explicit relational semantics through claims, premises, rebuttals, or typed edges~\cite{DWYER201016,A_Survey_on_Knowledge_Graphs,graphologue}. StructWeaver adopts a tree-based workspace not because readers' understanding is assumed to be inherently hierarchical, but because inquiry during AI-assisted reading often begins from a local point of interest and develops incrementally through follow-up questions and branching exploration~\cite{pirolli2005sensemaking,forage_chi26}. A simple hierarchy makes these paths visible while requiring relatively little additional representational knowledge from readers who are already working to understand an unfamiliar document.
...
\bibliography{sample-base,related_work}
...
This appendix provides pseudocode for the Gist--Verbatim construction pipeline referenced in Section~\ref{sec:implementation}.
...
Inspired by evaluation practices in natural language processing, such as ROUGE for summarization \cite{lin2005recall} and structured information extraction (e.g., triplet-based representations), we adopted a structure-aware evaluation of participant-written summaries (see also \S\ref{sec:summary-eval} and Table~\ref{tab:summary_quality}).

Two authors independently extracted core relational triples from each source paper and retained their intersection as the reference set. Participant summaries were segmented into semantic units, converted into triples, and scored for coverage and factual support.
...
\begin{itemize}[noitemsep, leftmargin=*]
 ...
 \item Summarization Time: Participants spent slightly more time summarizing with the system, increasing from $M = 7.23$ ($\text{Baseline}$) to $M = 7.93$ ($\text{System}$) ($\Delta = +0.71$). This difference was not statistically significant ($p=0.343$) and corresponded to a small effect size ($d_z=0.29$).
